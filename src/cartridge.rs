...
impl Rom {
    /// # Errors
    ///
    /// Will return `Err` if provided data does not have valid iNes v1 format
...
pub mod test {
    ...
    fn create_rom(rom: TestRom) -> Vec<u8> {
        let mut result = Vec::with_capacity(
            rom.header.len()
                + rom.trainer.as_ref().map_or(0, std::vec::Vec::len)
                + rom.pgp_rom.len()
                + rom.chr_rom.len(),
        );
    ...
    /// # Panics
    ///
    /// Might panic bu should not :D
    #[must_use]
    pub fn build_test_rom() -> Rom {
