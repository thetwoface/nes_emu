use crate::{cartridge::Rom, cpu::Mem, joypad::Joypad, ppu::NesPPU, ppu::PPU};
...
impl<'a> Bus<'a> {
    ...
    fn read_prg_rom(&self, mut addr: u16) -> u8 {
        ...
        if self.prg_rom.len() == 0x4000 && addr >= 0x4000 {
            ...
            // the upper 16 KiB needs to be mapped to the lower 16 KiB
...
impl Default for Bus<'_> {
    fn default() -> Self {
        Bus::new(Rom::default(), |_ppu: &NesPPU, _joypad| {})
    }
}
...
impl Mem for Bus<'_> {
    fn mem_read(&mut self, addr: u16) -> u8 {
        match addr {
            ...
            _ => {
                println!("Ignoring mem access at {addr:x}");
                0
            }
        }
    }
    ...
    fn mem_write(&mut self, addr: u16, data: u8) {
        match addr {
            ...
            0x8000..=0xFFFF => panic!("Attempt to write to Cartridge ROM space: {addr:x}"),

            _ => {
                println!("Ignoring mem write-access at {addr:x}");
            }
        }
    }
}
...
mod test {
    ...
    fn test_mem_read_write_to_ram() {
        let mut bus = Bus::new(test::build_test_rom(), |_ppu: &NesPPU, _joypad| {});
